...
class StringLexer extends Lexer {
  ...
  type LexerOutputType = Tokens[String]
  ...
  private[this] def parseImpl(aSource: LexerInputType, aStartIdx: Int, aAccumulatedTokens: Seq[Token[String]]): Either[LexerOutputType, LexerErrorType] = {
    if(aStartIdx > aSource.length-1) {
      return Left(Tokens(aAccumulatedTokens))
    }

    def constructToken(aSource: String, aStartIdx: Int, aTokenType: TokenType, aShouldIncludeToToken: Char => Boolean): (Int, Token[String]) = {
      def impl(aTokenStr: String, aSource: String, aCurrentIdx: Int, aTokenType: TokenType, aShouldIncludeToToken: Char => Boolean): (Int, Token[String]) = {
    ...
    def parseNumericToken(aSource: String, aStartIdx: Int): (Int, Either[Token[String], LexerErrorType]) = {
      val (nextIdx, token) = constructToken(aSource, aStartIdx, NumericTokenType, {ch =>
        ch.isDigit
      })
      return (nextIdx, Left(token))
    }
    def parseMulOperatorToken(aSource: String, aStartIdx: Int): (Int, Either[Token[String], LexerErrorType]) = {
      val (nextIdx, token) = constructToken(aSource, aStartIdx, MulOperatorTokenType, {ch =>
        MulOperatorTokenType.isValid(ch)
      })
      return (nextIdx, Left(token))
    }
    def parseDivOperatorToken(aSource: String, aStartIdx: Int): (Int, Either[Token[String], LexerErrorType]) = {
      val (nextIdx, token) = constructToken(aSource, aStartIdx, DivOperatorTokenType, {ch =>
        DivOperatorTokenType.isValid(ch)
      })
      return (nextIdx, Left(token))
    }
    def parsePlusOperatorToken(aSource: String, aStartIdx: Int): (Int, Either[Token[String], LexerErrorType]) = {
      val (nextIdx, token) = constructToken(aSource, aStartIdx, PlusOperatorTokenType, {ch =>
        PlusOperatorTokenType.isValid(ch)
      })
      return (nextIdx, Left(token))
    }
    def parseMinusOperatorToken(aSource: String, aStartIdx: Int): (Int, Either[Token[String], LexerErrorType]) = {
      val (nextIdx, token) = constructToken(aSource, aStartIdx, MinusOperatorTokenType, {ch =>
        MinusOperatorTokenType.isValid(ch)
      })
      return (nextIdx, Left(token))
    }
    def parseSpaceToken(aSource: String, aStartIdx: Int): (Int, Either[Token[String], LexerErrorType]) = {
    ...
    return tTokenOrError match {
      ...
      case Left(token: Token[String]) => parseImpl(aSource, tNextIdx, aAccumulatedTokens :+ token)
    }
  }
}
